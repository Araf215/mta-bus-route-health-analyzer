...
## How It Works
1. `scripts/fetch_data.py` requests live vehicle monitoring data from the MTA Bus Time API at a fixed interval.
2. Raw bus snapshots are stored in JSONL format in `data/raw/all_buses_snapshots.jsonl`.
3. `scripts/process_data.py` reads recent snapshots, compares consecutive bus positions using the Haversine formula, estimates movement rates, groups buses by borough, route, and direction, and computes route-level metrics.
4. `scripts/metrics.py` converts those route-level ratios into interpretable subscores and an overall Route Health score.
5. Processed output is written to `data/processed/all_routes_live_metrics.json`.
6. `web/app.js` loads the processed JSON and renders the dashboard with route filters, summary cards, smoothed trend charts, a live map, and a route-level vehicle table.
...
## Route Metrics
**In-Service Ratio**  
The proportion of detected buses on a route that appear to be actively serving riders rather than being identified as in layover.

**Movement Ratio**  
The proportion of in-service buses with valid movement data that meet the minimum movement rate threshold between snapshots. Movement is estimated from changes in latitude and longitude positions rather than the official schedule.

**Layover Ratio**  
The proportion of detected buses that are currently identified as being in layover.
...
3. Create a `.env` file based on the example file, add your MTA API key, and set `POLL_INTERVAL_SECONDS=300` for 5-minute snapshots.
4. Run the data collection script from the project root: `python scripts/fetch_data.py`
5. After the first raw snapshot is written to `data/raw/`, run the data processing script in a separate terminal: `python scripts/process_data.py`
6. Start a local server from the project root with `python -m http.server 8000` on another terminal. At this stage you should have 3 terminals (fetching, processing, and running the server).
7. Open `http://localhost:8000/web/` in your browser.
