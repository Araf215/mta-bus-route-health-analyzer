...
// Frontend refresh timing should match the local Python backend.
// The Python scripts collect and process new data every 5 minutes.
const REFRESH_MS = 300000;        // refresh every 5 minutes
const CHART_WINDOW_MINUTES = 30;  // show last 30 minutes
const CHART_BUCKET_MINUTES = 5;   // one bucket per backend update

// These should match the Python backend configuration.
// A bus is only counted as moving if it reaches this movement rate.
// Trend values are smoothed using this many recent snapshots.
const BACKEND_POLL_INTERVAL_MINUTES = 5;
...
async function loadLiveMetrics() {
  // The open-source version reads the JSON generated by process_data.py
  const response = await fetch(
    "/data/processed/all_routes_live_metrics.json",
    { cache: "no-store" }
  );
...
function renderScoreBox(routeData) {
  ...
  document.getElementById("scoreBox").innerHTML = `
   ...
   <div class="score-line"><strong>Route Health:</strong> ${formatNumber(live.health_score, 1)} / 10 (${live.health_label})</div>
    ...
    <div class="score-line"><strong>Trend Smoothing:</strong> Charts use the median of the last ${SMOOTHING_WINDOW_SNAPSHOTS} backend snapshots</div>
...
function renderSystemStatus(current) {
  const live = getCurrentDerivedMetrics(current); // Get the latest/raw route metrics
  const container = document.getElementById("systemStatus"); // Route Status section in the HTML
  const notes = []; // Stores each status message before displaying them

  // Adds the current Route Health score and label
  notes.push(
    `Health score is currently rated as ${live.health_label}, with a Route Health score of ${formatNumber(live.health_score, 1)} out of 10.`
  );

  // Shows how many detected buses are currently counted as in service
  if (live.total_vehicles != null && live.in_service_vehicles != null) {
    notes.push(
      `${formatInteger(live.in_service_vehicles)} of ${formatInteger(live.total_vehicles)} detected buses are currently being counted as in service.`
    );
  }

  // Shows how many buses are currently identified as being in layover
  if (live.layover_vehicles != null) {
    const layoverCount = Number(live.layover_vehicles); // Converts the layover count into a number

    notes.push(
      `${formatInteger(layoverCount)} ${layoverCount === 1 ? "bus is" : "buses are"} currently flagged as being in layover.`
    );
  }

  // Shows how many buses meet the minimum movement-rate threshold
  if (live.moving_vehicles != null) {
    const movingCount = Number(live.moving_vehicles); // Converts the moving bus count into a number

    notes.push(
      `${formatInteger(movingCount)} ${movingCount === 1 ? "bus is" : "buses are"} currently counted as moving using a ${MOVING_THRESHOLD_METERS_PER_MINUTE} m/min threshold.`
    );
  }

  // Shows the average distance buses moved between the recent snapshots
  if (live.average_moved_meters != null) {
    notes.push(
      `The average detected bus moved about ${formatMeters(live.average_moved_meters)} between recent snapshots.`
    );
  }

  // Turns each status message into a status-pill div and add them to the page
  container.innerHTML = notes
    .map(note => `<div class="status-pill">${note}</div>`)
    .join("");
}

// Explains how the route health score is calculated.
// This is intentionally more transparent about both formulas and assumptions.
function renderScoreBreakdown(current) {
  const live = getCurrentDerivedMetrics(current);
  const score = live.score_breakdown || current.score_breakdown || {};
  ...
  container.innerHTML = `
    ...
    <div class="metric-pill">
     ...
     Current route snapshot values and the current Route Health score use the latest available snapshot. Trend graphs use smoothed values based on the median of the last ${SMOOTHING_WINDOW_SNAPSHOTS} backend snapshots to reduce short-term API noise. </div>
  `;
}
...
function renderMethodText() {
  document.getElementById("methodText").innerHTML = `
   ...
   The current snapshot shows the latest available route data. To reduce short-term API noise in the trend graphs, chart values are smoothed using the median of the last ${SMOOTHING_WINDOW_SNAPSHOTS} processed snapshots.
  `;
}
